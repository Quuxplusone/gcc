...
_GLIBCXX_BEGIN_NAMESPACE_VERSION
  ...
  struct __destroy_guard {
    _Tp *__t_;
    _GLIBCXX20_CONSTEXPR explicit __destroy_guard(_Tp *__t) : __t_(__t) { }
    _GLIBCXX20_CONSTEXPR ~__destroy_guard() { std::_Destroy(__t_); }
  };
  ...
  _GLIBCXX20_CONSTEXPR inline _Dt *__libcpp_relocate_at2(long, _St *__source, _Dt *__dest)
  ...
  template<class _St, class _Dt, typename enable_if<
      ...
      int>::type = 0>
  _GLIBCXX20_CONSTEXPR inline _Dt *__libcpp_relocate_at2(int, _St *__source, _Dt *__dest) _GLIBCXX_NOEXCEPT {
    ::__builtin_memmove(__dest, __source, sizeof(_St));
    return __dest;
  }

  template<class _Tp>
  _GLIBCXX20_CONSTEXPR inline _Tp *relocate_at(_Tp *__source, _Tp *__dest)
      _GLIBCXX_NOEXCEPT_IF(is_nothrow_move_constructible<_Tp>::value && is_nothrow_destructible<_Tp>::value)
  {
    ...
    if constexpr (__use_memcpy) {
      if (!std::__is_constant_evaluated()) {
        return std::__libcpp_relocate_at2_impl(0, __source, __dest);
      }
    }
    return std::__libcpp_relocate_at2(0L, __source, __dest);
#else
